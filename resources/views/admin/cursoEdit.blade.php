<x-layout>
    <x-slot:title>Editar Curso - {{ $curso->nombre }}</x-slot:title>
    ...
    <main class="container mx-auto px-6 py-16">
        <div class="max-w-4xl mx-auto mb-12 animate-fade-in">
            ...
            <h2 class="text-5xl font-serif text-slate-900 leading-tight">Editar curso</h2>
            <p class="text-slate-500 mt-2 font-light">Modifica los datos del curso.</p>
        </div>
        ...
        <form action="{{ route('curso.update', ['id' => $curso->id]) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="space-y-8">
                    <div class="group flex flex-col gap-2">
                        ...
                        <input type="text" name="nombre" id="nombre"
                            ...
                            placeholder="Ej: Master en Laravel Avanzado" value="{{ old('nombre', $curso->nombre) }}">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="group flex flex-col gap-2">
                            ...
                            <input type="number" name="duracion" id="duracion"
                                ...
                                placeholder="120" value="{{ old('duracion', $curso->duracion) }}">
                        ...
                        <div class="group flex flex-col gap-2">
                            ...
                            <select name="nivel" id="nivel" 
                                ...
                                <option value="" disabled {{ old('nivel', $curso->nivel) ? '' : 'selected' }}>Seleccionar</option>
                                <option value="1" {{ old('nivel', $curso->nivel) == '1' ? 'selected' : '' }}>1 - Básico</option>
                                <option value="2" {{ old('nivel', $curso->nivel) == '2' ? 'selected' : '' }}>2 - Medio</option>
                                <option value="3" {{ old('nivel', $curso->nivel) == '3' ? 'selected' : '' }}>3 - Pro</option>
                            </select>
                ...
                <div class="space-y-8">
                    <div class="group flex flex-col gap-3">
                        ...
                        <div class="relative overflow-hidden rounded-[2rem] bg-slate-50 border-2 border-dashed border-slate-200 p-8 transition-all hover:border-indigo-400 text-center">
                            ...
                            @if($curso->imagen)
                                <p class="mt-4 text-xs text-slate-400">Imagen actual: <span class="font-bold text-slate-800">{{ $curso->imagen }}</span></p>
                                <img src="{{ asset($curso->imagen) }}" class="h-24 w-40 object-cover rounded-2xl shadow-sm border border-slate-100 mt-3 mx-auto">
                            @endif
                        </div>
            ...
            <div class="group flex flex-col gap-2">
                ...
                <textarea id="descripcion" name="descripcion" rows="6"
                    ...
                    placeholder="Contenido del curso...">{{ old('descripcion', $curso->descripcion) }}</textarea>
